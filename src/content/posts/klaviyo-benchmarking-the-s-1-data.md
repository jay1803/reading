---
title: "Klaviyo: Benchmarking the S-1 Data"
date: 2023-10-10T18:52:00Z
category: reading
description: "This enables business users of any skill level to harness their data in order to send the right message at the right time across email, SMS, and push notific..."
source: "https://cloudedjudgement.substack.com/p/klaviyo-benchmarking-the-s-1-data"
---

### Klaviyo Overview
This enables business users of any skill level to harness their data in order to send the right message at the right time across email, SMS, and push notifications, more accurately measure and predict performance, and deploy the specific actions and campaigns that drive the highest impact.
### Product Overview
Klaviyo 的核心是一个营销自动化解决方案，涵盖电子邮件、短信、移动推送和评论，将所有这些都整合到一个客户数据平台中。客户可以对他们的客户进行细分，创建覆盖多个渠道的特定活动，并通过 Klayvio 平台更有效地与受众进行整体互动
### Market Opportunity
the customers we serve primarily operate within the retail vertical, with retailers spanning both online and offline channels. Our estimated serviceable addressable market opportunity within this vertical is over $16 billion.

我们相信我们的平台在广泛的垂直领域具有高度可扩展性，包括教育、活动和娱乐、餐厅和旅游以及 B2B 公司。
我们估计，仅在美国，我们的平台在所有这些垂直领域的潜在市场机会总额就达到 340 亿美元。
### How Klaviyo Makes Money
Klaviyo has about 130k customers, with an ACV of ~$5k

From the S-1: “We generate revenue through the sale of subscriptions to our customers for the use of our platform. Our subscription plans are tiered based on the number of active consumer profiles stored on our platform and the number of emails and SMS messages sent.

As our customers’ businesses grow, they utilize more active consumer profiles and send more emails and SMS messages, which naturally increases their usage of our platform. Our revenue also expands when our customers add additional channels, such as SMS, and additional use cases, such as reviews and our CDP offering, or when their other brands, business units, and geographies start using the platform.
### Benchmark Data
#### Last Twelve Months (LTM) Revenue
Klaviyo’s LTM revenue was $585M

#### LTM Revenue Growth
Klaviyo grew revenue 57% over the last 12 months

#### Quarterly YoY Revenue Growth Trends
Klaviyo has maintained impressive growth

#### LTM GAAP Gross Margin
Klaviyo’s gross margins are 75%

#### LTM GAAP Operating Margin
Klaviyo’s operating margins are (4%). In their last 2 quarters they did have a positive GAAP operating margin (1% and 4%)
#### LTM FCF Margin
Klaviyo’s LTM FCF margins are 8%
#### Net Revenue Retention
Klaviyo’s net retention is 119%

#### Gross Margin Adjusted CAC Payback
Klaviyo’s CAC payback period is 29 months

#### LTM S&M Expense as % of LTM Revenue
Klaviyo spent 42% of revenue on S&M over the last 12 months

Quarterly S&M trends below

#### Rule of 40
Klaviyo’s rule of 40 is one of the best at 65%
