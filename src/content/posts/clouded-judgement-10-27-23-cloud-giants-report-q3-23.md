---
title: "Clouded Judgement 10.27.23 - Cloud Giants Report Q3 '23"
date: 2023-10-28T21:29:50Z
category: reading
author: "Jamin Ball"
description: "云巨头（AWS、Azure 和 Google Cloud）对于软件来说并不是一个很好的信号……第二季度（3 个月前）之后，云巨头围绕优化的基调主要是：优化已经开始放松，并净增新的工作负载已拾取。"
source: "https://cloudedjudgement.substack.com/p/clouded-judgement-102723-cloud-giants"
---

### Cloud Giants Report Q3 ‘23
云巨头（AWS、Azure 和 Google Cloud）对于软件来说并不是一个很好的信号……第二季度（3 个月前）之后，云巨头围绕优化的基调主要是：优化已经开始放松，并净增新的工作负载已拾取。

我相信，尽管软件在 10 年里增长了 5%，但仍能保持不变的更大原因之一是对重新加速的预期（即数字/预测上升）。要么就是人们预计 10 年期利率会回落​​至 3.5%，但感觉不太可能。

Here’s a broad update on AWS, Azure and Google Cloud:
AWS (Amazon): $92B run rate growing 12% YoY (last Q grew 12%)
Azure (Microsoft): ~$66B run rate (estimate) growing 28% YoY (last Q grew 27%)
Google Cloud (includes GSuite): $34B run rate growing 22% YoY (last Q grew 28%, neither are cc)

Azure 也从其 AI 服务中受益匪浅。他们称本季度 3% 的增长来自 Azure AI，这意味着 Azure AI 的运行速度为 1.5B 美元！上季度 Azure AI 的运行费用为 5 亿美元。环比 200% 的增长令人印象深刻。
除去 AI 之外，Azure 同比增长约 25%（恒定货币增长率为 28%，AI 的增长率减去 3%）。上季度，Azure ex AI 同比增长 26%（按固定汇率计算增长 27%，其中 1% 来自人工智能）。
### Quarterly Reports Summary
### Top 10 EV / NTM Revenue Multiples

### Update on Multiples
EV/Revenue multiples
Overall Median: 5.4x
Top 5 Median: 12.6x
10Y: 4.9%

在下面的类别中，我认为高增长 >30% 预计 NTM 增长、中增长 15%-30% 和低增长 <15%
High Growth Median: 11.0x
Mid Growth Median: 7.5x
Low Growth Median: 3.4x
### EV / NTM Rev / NTM Growth
下图显示了 EV/NTM 收入倍数除以 NTM 共识增长预期。

### EV / NTM FCF
### Scatter Plot of EV / NTM Rev Multiple vs NTM Rev Growth
How correlated is growth to valuation multiple?

### Operating Metrics
Median NTM growth rate: 15%
Median LTM growth rate: 21%
Median Gross Margin: 75%
Median Operating Margin (18%)
Median FCF Margin: 7%
Median Net Retention: 114%
Median CAC Payback: 35 months
Median S&M % Revenue: 42%
Median R&D % Revenue: 27%
Median G&A % Revenue: 16%
